    ...
    <>
        ...
        {tiles.map((tile, idx) => (
          ...
          >
            <MediaFrame
              ...
              ratio={tile.ratio}
              src={tile.slot}
              alt={`Acemco — gallery image ${idx + 1}`}
              zoom
      ...
      {open !== null && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-pub-espresso/95 p-4" onClick={() => setOpen(null)}>
          <button type="button" className="absolute right-5 top-5 rounded-full p-2 text-pub-on-dark hover:bg-white/10" aria-label="Close" onClick={() => setOpen(null)}>
            <X size={26} />
          </button>
          <button
            type="button"
          ...
          >
            <MediaFrame slot={`gallery.${open + 1}`} background src={tiles[open].slot} alt={`Acemco — gallery image ${open + 1}`} sizes="(max-width: 768px) 88vw, 440px" />
          </div>
          <button
            type="button"
